...
if (nrow(especies_pendentes) == 0) {
  ...
  parametros_execucao <- data.frame(
    parametro = c(
      ...
      'background_min','background_max','usar_cobertura_arborea','na_cobertura_strategy','metodos_modelagem',
      'modo_execucao','especie_unica','especies_lista','especie_partida'
    ),
    valor = c(
      lote_tamanho, pausa_minutos, max_tentativas, n_cores_inicial, n_cores, safe_mode, limiar_vif, n_replicacoes, test_percent,
      background_min, background_max, usar_cobertura_arborea, na_cobertura_strategy, paste(metodos_modelagem, collapse = ','),
      modo_execucao,
